...
const projects=[
{name:'Sklep internetowy MODÉA',type:'Sklep internetowy',url:'/portfolio/sklep-moda.html',desc:'Nowoczesny sklep z produktami, koszykiem i prezentacją kolekcji.'},
{name:'Restauracja La Tavola',type:'Strona restauracji',url:'/portfolio/restauracja.html',desc:'Menu, galeria, kontakt i rezerwacja stolika online.'},
{name:'NovaBud',type:'Strona firmowa',url:'/portfolio/strona-firmowa.html',desc:'Profesjonalna strona usługowa z ofertą, realizacjami i formularzem wyceny.'},
{name:'FlowDesk',type:'Aplikacja webowa',url:'/portfolio/aplikacja-saas.html',desc:'Panel SaaS z dashboardem, projektami, klientami i danymi biznesowymi.'},
{name:'Studio Belle',type:'Rezerwacje online',url:'/portfolio/rezerwacje.html',desc:'Strona salonu z usługami, cennikiem i systemem rezerwacji wizyt.'}
];
...
function Home({go}){return <main><section className="hero"><div><span className="pill">STRONY • SKLEPY • APLIKACJE • LOGO • QA</span><h1>Tworzymy strony, sklepy i aplikacje <em>dla Twojej firmy.</em></h1><p>Od pomysłu do działającego projektu. Wypełniasz formularz, klikasz „Wyślij”, a wiadomość trafia bezpośrednio do ProjektKreator.pl bez otwierania programu pocztowego.</p><div className="buttons"><button className="primary" onClick={()=>go('zamow')}>Rozpocznij projekt <ArrowRight/></button><button onClick={()=>go('realizacje')}>Nasze realizacje</button></div><div className="checks"><span><Check/> Mobile</span><span><Check/> SEO podstawowe</span><span><Check/> Jasna wycena</span><span><Check/> Formularz online</span></div></div><div className="heroLogo"><iframe src="/portfolio/sklep-moda.html" title="Przykładowy sklep internetowy" style={{width:'145%',height:'590px',border:0,transform:'scale(.72)',transformOrigin:'center',pointerEvents:'none'}}/><b>🛍️ Sklep internetowy od ProjektKreator.pl</b><b>✨ Tak może wyglądać Twój projekt</b></div></section><section className="stats"><div><b>599 PLN</b><span>strona od</span></div><div><b>149 PLN</b><span>debug od</span></div><div><b>249 PLN</b><span>logo od</span></div><div><b>5</b><span>przykładowych realizacji</span></div></section><section className="section"><Title k="USŁUGI" h="Wybierz to, czego potrzebujesz." p="Możemy zbudować projekt od zera albo poprawić to, co już masz."/><Cards go={go}/></section><section className="section dark"><Title k="PRZYKŁADOWE REALIZACJE" h="Zobacz, jakie strony możemy stworzyć." p="Sklep internetowy, restauracja, strona firmowa, aplikacja webowa i system rezerwacji."/><Moving/></section><section className="section"><Title k="JAK TO DZIAŁA" h="Prosto i bez zakładania konta."/><div className="steps">{[['01','Wybierasz usługę','Opisujesz, czego potrzebujesz.'],['02','Wypełniasz formularz','Dodajesz kontakt i opcjonalne pliki.'],['03','Klikasz Wyślij','Wiadomość trafia bezpośrednio do nas.'],['04','Odpowiadamy','Potwierdzamy zakres, cenę i termin e-mailem.']].map(x=><article key={x[0]}><b>{x[0]}</b><h3>{x[1]}</h3><p>{x[2]}</p></article>)}</div></section></main>}
...
function Portfolio(){return <main className="darkPage"><section className="pageHead"><span>PRZYKŁADOWE REALIZACJE</span><h1>Zobacz, co możemy stworzyć dla Twojej firmy.</h1><p>Pięć różnych typów stron i aplikacji pokazanych jako działające demonstracje.</p></section><section className="section"><Moving/><div className="portfolioList">{projects.map(p=><a href={p.url} target="_blank" rel="noreferrer" key={p.name}><iframe src={p.url} title={p.name}/><div><span>{p.type}</span><h3>{p.name}</h3><p>{p.desc}</p><b>Otwórz demo <ExternalLink size={15}/></b></div></a>)}</div></section></main>}
