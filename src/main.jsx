...
function Cards({go}){return <div className="cards">{services.map(s=><article key={s.id}><div className="service-image"><img src={serviceImages[s.id]} alt="" loading="lazy"/></div><div className="service-card-body"><i>{s.icon}</i><h3>{s.name}</h3><p>{s.desc}</p><strong>od {money(s.price)}</strong><button onClick={()=>go('uslugi')}>Dowiedz się więcej <ArrowRight size={15}/></button></div></article>)}</div>}
