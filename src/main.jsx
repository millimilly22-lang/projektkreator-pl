...
import{ArrowRight,Mail,Check,Menu,X,Paperclip}from'lucide-react';
import{projects}from'./portfolio.js';
import{MovingPortfolio}from'./components/LivePortfolio.jsx';
import ProjektKreatorHeroVisual from'./components/ProjektKreatorHeroVisual.jsx';
import'./styles.css';
...
const serviceImages={landing:'https://images.unsplash.com/photo-1496181133206-80ce9b88a853?auto=format&fit=crop&w=900&q=85',business:'https://images.unsplash.com/photo-1558655146-d09347e92766?auto=format&fit=crop&w=900&q=85',pro:'https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=900&q=85',shop:'https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=900&q=85',webapp:'https://images.unsplash.com/photo-1515879218367-8466d910aaa4?auto=format&fit=crop&w=900&q=85',debug:'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=900&q=85',testing:'https://images.unsplash.com/photo-1553877522-43269d4ea984?auto=format&fit=crop&w=900&q=85',logo:'https://images.unsplash.com/photo-1561070791-2526d30994b5?auto=format&fit=crop&w=900&q=85',automation:'https://images.unsplash.com/photo-1535378917042-10a22c95931a?auto=format&fit=crop&w=900&q=85'};
...
function Home({go}){return <main><section className="hero"><div><span className="pill">STRONY • SKLEPY • APLIKACJE • LOGO • QA</span><h1>Tworzymy strony, sklepy i aplikacje <em>dla Twojej firmy.</em></h1><p>Od pomysłu do działającego projektu. Wypełniasz formularz, klikasz „Wyślij”, a wiadomość trafia bezpośrednio do ProjektKreator.pl bez otwierania programu pocztowego.</p><div className="buttons"><button className="primary" onClick={()=>go('zamow')}>Rozpocznij projekt <ArrowRight/></button><button onClick={()=>go('realizacje')}>Nasze realizacje</button></div><div className="checks"><span><Check/> Mobile</span><span><Check/> SEO podstawowe</span><span><Check/> Jasna wycena</span><span><Check/> Formularz online</span></div></div><div className="heroLogo"><ProjektKreatorHeroVisual/></div></section><section className="stats"><div><b>599 PLN</b><span>strona od</span></div><div><b>149 PLN</b><span>debug od</span></div><div><b>249 PLN</b><span>logo od</span></div><div><b>3</b><span>działające realizacje</span></div></section><section className="section"><Title k="USŁUGI" h="Wybierz to, czego potrzebujesz." p="Możemy zbudować projekt od zera albo poprawić to, co już masz."/><Cards go={go}/></section><section className="section dark"><Title k="PRZYKŁADOWE REALIZACJE" h="Zobacz, jakie strony możemy stworzyć." p="MODÉA, La Tavola i NovaBud — trzy działające wersje demonstracyjne, które możesz otworzyć i przetestować."/><MovingPortfolio/></section><section className="section"><Title k="JAK TO DZIAŁA" h="Prosto i bez zakładania konta."/><div className="steps">{[['01','Wybierasz usługę','Opisujesz, czego potrzebujesz.'],['02','Wypełniasz formularz','Dodajesz kontakt i opcjonalne pliki.'],['03','Klikasz Wyślij','Wiadomość trafia bezpośrednio do nas.'],['04','Odpowiadamy','Potwierdzamy zakres, cenę i termin e-mailem.']].map(x=><article key={x[0]}><b>{x[0]}</b><h3>{x[1]}</h3><p>{x[2]}</p></article>)}</div></section><section className="contactTeaser"><div><span>KONTAKT</span><h2>Porozmawiajmy o Twoim projekcie.</h2><p>Masz pytanie albo gotowy pomysł? Napisz do nas — odpowiemy i przygotujemy jasną wycenę.</p></div><div className="contactTeaserDetails"><a href="mailto:liashany4@gmail.com">liashany4@gmail.com</a><span>ProjektKreator.pl · Polska</span><button className="primary" onClick={()=>go('kontakt')}>Napisz do nas <ArrowRight size={16}/></button></div></section></main>}
const Title=({k,h,p})=><div className="title"><span>{k}</span><h2>{h}</h2>{p&&<p>{p}</p>}</div>;
function Cards({go}){return <div className="cards">{services.map(s=><article key={s.id}><div className="service-image"><img src={serviceImages[s.id]} alt=""/></div><div className="service-card-body"><i>{s.icon}</i><h3>{s.name}</h3><p>{s.desc}</p><strong>od {money(s.price)}</strong><button onClick={()=>{sessionStorage.setItem('service',s.id);go('zamow')}}>Dowiedz się więcej <ArrowRight size={15}/></button></div></article>)}</div>}
...
function Portfolio(){return <main className="darkPage"><section className="pageHead"><span>PRZYKŁADOWE REALIZACJE</span><h1>Zobacz, co możemy stworzyć dla Twojej firmy.</h1><p>Trzy działające demonstracje: sklep MODÉA, restauracja La Tavola i strona firmy NovaBud.</p></section><section className="section"><MovingPortfolio/><div className="portfolioLinks">{projects.map(p=><a href={p.url} target="_blank" rel="noreferrer" key={p.id}><span>{p.type}</span><h3>{p.name}</h3><p>{p.desc}</p><b>Otwórz demo <ArrowRight size={15}/></b></a>)}</div></section></main>}
